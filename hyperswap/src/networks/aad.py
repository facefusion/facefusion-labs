...
class AdaptiveFeatureModulation(nn.Module):
	def __init__(self, input_channels: int, output_channels: int, target_channels: int, source_channels: int, num_blocks: int) -> None:
		super().__init__()
		self.primary_feature_mods = nn.ModuleList()
		self.primary_relus = nn.ModuleList()
		self.primary_convs = nn.ModuleList()

		current_in_channels = input_channels
		for index in range(num_blocks):
			self.primary_feature_mods.append(
				FeatureModulation(current_in_channels, target_channels, source_channels)
			)
			self.primary_relus.append(nn.ReLU())
			if index < num_blocks - 1:
				conv = nn.Conv2d(current_in_channels, current_in_channels, kernel_size=3, padding=1, bias=False)
			else:
				conv = nn.Conv2d(current_in_channels, output_channels, kernel_size=3, padding=1, bias=False)
			self.primary_convs.append(conv)

		self.shortcut_feature_mod: nn.Module | None = None
		self.shortcut_relu: nn.Module | None = None
		self.shortcut_conv: nn.Module | None = None
		if input_channels > output_channels:
			self.shortcut_feature_mod = FeatureModulation(input_channels, target_channels, source_channels)
			self.shortcut_relu = nn.ReLU()
			self.shortcut_conv = nn.Conv2d(input_channels, output_channels, kernel_size=3, padding=1, bias=False)

	def forward(self, input_tensor: Tensor, source_embedding: Embedding, target_feature: Feature) -> Tensor:
		if self.shortcut_feature_mod and self.shortcut_relu and self.shortcut_conv:
			shortcut_tensor = self.shortcut_feature_mod(input_tensor, source_embedding, target_feature)
			shortcut_tensor = self.shortcut_relu(shortcut_tensor)
			shortcut_tensor = self.shortcut_conv(shortcut_tensor)
		else:
			shortcut_tensor = input_tensor

		primary_tensor = input_tensor
		for i in range(len(self.primary_convs)):
			primary_tensor = self.primary_feature_mods[i](primary_tensor, source_embedding, target_feature)
			primary_tensor = self.primary_relus[i](primary_tensor)
			primary_tensor = self.primary_convs[i](primary_tensor)
		return primary_tensor + shortcut_tensor
...
class FeatureModulation(nn.Module):
	def __init__(self, input_channels: int, target_channels: int, source_channels: int) -> None:
		...
		self.conv1: nn.Module = nn.Conv2d(target_channels, input_channels, kernel_size=1)
		self.conv2: nn.Module = nn.Conv2d(target_channels, input_channels, kernel_size=1)
		self.conv3: nn.Module = nn.Conv2d(input_channels, 1, kernel_size=1)
		self.linear1: nn.Module = nn.Linear(source_channels, input_channels)
		self.linear2: nn.Module = nn.Linear(source_channels, input_channels)
		self.instance_norm: nn.Module = nn.InstanceNorm2d(input_channels)
